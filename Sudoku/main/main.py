...
def gen_save(num, sym, sourceimg, destination):
    ...
    print(num1, "very Easy, ", num2, "Easy, ", num3, "Medium, ",
          num4, "Hard and ", num5, "very Hard sudoku generated")
    ...
    def onPressRate():
        ...
        if (np.min(A) >= 0 and np.max(A) <= 9):
            if (SC.IsSudoRight(A)):
                ...
                if (GU.CountSolutions(Adef, [], 0, 1) == 1):
                    diff = RT.RateProb(A)
                    printMes("Rating", "Your Problem is " +
                             BS.getStrDiff(difficulty))
                else:
...
def convertMatrixBoardToStringBoard(mat):
    stringBoard = ""
    for i in range(9):
        for j in range(9):
            if mat[i][j] == 0:
                ...
                stringBoard += str(mat[i][j])
    return stringBoard


def convertStringBoardToMatrixBoard(mat):
    matBoard = np.zeros((9, 9), dtype=int)
    stringIndex = 0
    for i in range(0, 9):
        for j in range(0, 9):
            char = mat[stringIndex]
            if char == "*":
                matBoard[i, j] = 0
            else:
                matBoard[i, j] = int(char)
            stringIndex += 1
    return matBoard


def generateBoards(numOfBoards, noOfCellsToBeFilled):
    result = []
    for i in range(9001, numOfBoards+1):
        outputDict = {}
        matBoard = Gen.RandGenerator([], noOfCellsToBeFilled, 1)
        outputDict["id"] = i
        outputDict["board"] = convertMatrixBoardToStringBoard(matBoard)
        outputDict["rating"] = RT.RateProb(matBoard)
        result.append(outputDict)
    return result


boards = []
with open("boards.json") as outfile:
    data = json.load(outfile)
    for board in data:
        matBoard = convertStringBoardToMatrixBoard(board["board"])
        solutionBoard = SL.SudoSolveIt2(matBoard, [], 1)
        board["solution"] = convertMatrixBoardToStringBoard(solutionBoard)
        boards.append(board)
    # print(list(duplicates(data)))

with open("boardsWithSols.json", "w")as f:
    json.dump(boards, f)
