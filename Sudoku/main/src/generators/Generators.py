...
# Top-dowm, simulated annealing with time limit
def RandGeneratorTimLim(A, Known, n, Abeg, tim):
    Arec = deepcopy(A)
    start2 = time.time()
    if (n == 1):
        Arec = GU.makeNewSudo(np.zeros((9, 9), dtype=int), 1)
        ...
        if (lenPos < 27):
            ...
            if (x <= p):
                for i in range(2):
                    ...
                    Arec[pos2fill[0, pick], pos2fill[1, pick]
                         ] = Abeg[pos2fill[0, pick], pos2fill[1, pick]]
                pos2erase = np.array(np.where(Arec != 0))
                lenPos = pos2erase[0, :].size
        while (lenPos > 0):
            A3 = deepcopy(Arec)
            ...
            A3[pos2erase[0, pick], pos2erase[1, pick]] = 0
            if (GU.CountSolutions(A3, [], 0, 1) == 1):
                A2 = RandGeneratorTimLim(
                    A3, Known, 2, Abeg, time.time()-start2+tim)
                if (A2[0][0] == -1):
                    return A2
                lenKn = np.array(np.where(A2 != 0))
                if(lenKn[0, :].size == Known):
...
def RandFillGen(A, Known, n, Abeg):  # Bottom-up
    ...
    if (lenKnown < Known-1):
        while (lenFill > 0):
            A2 = deepcopy(Arec)
            pick = randint(0, lenFill-1)
            A2[pos2fill[0, pick], pos2fill[1, pick]
               ] = Abeg[pos2fill[0, pick], pos2fill[1, pick]]
